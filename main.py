import pygame
import asyncio  # NEW: needed so the browser can run the loop cooperatively
import os
import random
...
# --- CHANGED: asset paths flattened to match files sitting next to main.py ---
# (pygbag serves everything in this folder to the browser, so no nested
#  "onkey_start/Assets/..." folders are needed)
JUMP_SOUND = pygame.mixer.Sound('jump.ogg')
DIE_SOUND = pygame.mixer.Sound('die.ogg')

FONT = pygame.font.Font('PixelOperator-Bold.ttf', 30)
SCORE_FONT = pygame.font.Font('PixelOperator-Bold.ttf', 23)
...
PLAY_JUMP = pygame.transform.scale(pygame.image.load('jump.png'), (PLAY_WIDTH, PLAY_HEIGHT))
PLAY_STANDING = pygame.transform.scale(pygame.image.load('stand.png'), (PLAY_WIDTH, PLAY_HEIGHT))
PLAY_LEFT = pygame.transform.scale(pygame.image.load('left.png'), (PLAY_WIDTH, PLAY_HEIGHT))
PLAY_RIGHT = pygame.transform.scale(pygame.image.load('right.png'), (PLAY_WIDTH, PLAY_HEIGHT))
...
CACTUS = pygame.image.load('plant.png').convert_alpha()
CACTUS_WIDTH = 50
...
async def game_loop():  # CHANGED: def -> async def
    ...
    while game_running:
        ...
        await asyncio.sleep(0)  # NEW: hands control back to the browser each frame

    pygame.quit()


if __name__ == '__main__':  # tells to only run when file is executed
    asyncio.run(game_loop())  # CHANGED: game_loop() -> asyncio.run(game_loop())
